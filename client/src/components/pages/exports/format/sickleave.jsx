...
const sickleave = ({userData, leaveData, userSignature,inspectorSignature,firstSignature,secondSignature, prevStat}) => {
  ...
  console.log("prevStat:", prevStat);
    ...
    pdf.text(`เรื่อง ขออนุญาตลาป่วย`, 25, 49);
    ...
    pdf.addImage(checked, "jpeg", 50, 79, 5.1, 5.1);
    ...
    pdf.addImage(blank, "jpeg", 50, 86, 5, 5);
    ...
    pdf.addImage(checked, "jpeg", 47, 106.2, 5.1, 5.1);
    pdf.addImage(blank, "jpeg", 65, 106.25, 5, 5);
    ...
    pdf.autoTable({
      ...
      body: [
        [
          "ลาป่วย",
          `${prevStat.SL_remaining}`,
          `${leaveData[0].numDay}`,
          `${prevStat.SL_remaining + leaveData[0].numDay}`,
        ],
        [
          "ลากิจส่วนตัว",
          `-`,
