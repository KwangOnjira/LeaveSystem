...
const vacationleave = ({userData, leaveData, userSignature,deputySignature,inspectorSignature,firstSignature,secondSignature, prevStat}) => {
  ...
  console.log("deputySignature:", deputySignature);
  ...
  console.log("prevStat:", prevStat);
    ...
    pdf.setFontSize(20);
    pdf.text(`ใบลาพักผ่อน`, width / 2, 23, {
      align: "center",
    });
    ...
    pdf.text(`เรื่อง ขออนุญาตลาพักผ่อน`, 25, 49);
    pdf.text(`เรียน ${leaveData[0].to}`, 25, 56);
    pdf.text(
      `                 ข้าพเจ้า.....................................................................................................................................................  ตำแหน่ง................................................................................................. สังกัด...............................................................  ฝ่าย..................................................................................................... มีวันลาพักผ่อนสะสม..........................วันทำการ มีสิทธิลาพักผ่อนประจำปีอีก.............วันทำงาน รวมเป็น..........วันทำงาน ขอลาพักผ่อนตั้งแต่วันที่............................... ถึงวันที่............................... มีกำหนด.............วัน ในระหว่างลาจะติดต่อข้าพเจ้าได้ที่.................................................`,
      25,
      63,
      { maxWidth: 170 }
    );
    ...
    pdf.text(
      `${prevStat.VL_accumulatedDays}`,
      166,
      75.5,
    );
    pdf.text(
      `${prevStat.leave_rights}`,
      70,
      82,
    );
    pdf.text(
    ...
    pdf.text("ขอแสดงความนับถือ", 115, 97);
    pdf.addImage(userSignature, "jpeg", 127, 102, 10, 5);
    pdf.text(
      `(ลงชื่อ)...............................................`,
      100,
      108
    ...
    pdf.setFont("MyFontBold", "bold");
    pdf.text("สถิติการลาในปีงบประมาณนี้", 27, 128);
    pdf.autoTable({
      tableWidth: 70,
      startY:130,
      head: [["ลามาแล้ว (วันทำการ)", "ลาครั้งนี้ (วันทำการ)", "รวม (วันทำการ)"]],
      body: [
        [
          `${prevStat.VL_total - prevStat.VL_remaining}`,
          `${leaveData[0].numDay}`,
          `${prevStat.VL_total - prevStat.VL_remaining + leaveData[0].numDay}`,
        ],
      ],
      styles:{fillColor:null,lineWidth:0.5,}
      ,
      headStyles: {textColor:`#000000`,
    ...
    pdf.setFont("MyFont", "normal");
    pdf.addImage(inspectorSignature, "jpeg", 50,157, 10, 5);
    pdf.text(
      `(ลงชื่อ)...............................................`,
      25,
      163
    ...
    pdf.setFontSize(14);
    pdf.text(
      `               ${leaveData[0].who_inspector.split('(')[1].trim().slice(0, -1)}`,
      20,
      174.5,{ maxWidth: 70 }
    );
    pdf.setFontSize(16);
    ...
    pdf.setFont("MyFontBold", "bold");
    pdf.text("หมายเหตุ", 42, 197);

    pdf.setFont("MyFont", "normal");
    pdf.text(
        `ในระหว่างที่ข้าพเจ้าลาพักผ่อนนี้ได้มอบหมายงานให้กับ`,
        15,
        205,
      );
    pdf.text(
    ...
    pdf.text(
      ...
      pdf.addImage(userSignature, "jpeg", 50,219, 10, 5);
    pdf.text(
      `(ลงชื่อ)...............................................ผู้มอบ`,
      25,
      225
    ...
    pdf.setFontSize(16);
      pdf.addImage(deputySignature, "jpeg", 50,246, 10, 5);
    pdf.text(
      `(ลงชื่อ)...............................................ผู้รับมอบ`,
      25,
      252
    ...
    pdf.setFontSize(14);
    pdf.text(
      `               ${leaveData[0].vacationleave.deputyName.split('(')[1].trim().slice(0, -1)}`,
      20,
      264.5,{ maxWidth: 70 }
    );
pdf.setFontSize(16);
pdf.text(`วันที่...............................................`, 25, 278);
    ...
    pdf.setFont("MyFontBold", "bold");
    pdf.text("ความเห็นของผู้บังคับบัญชา", 124, 140);
    ...
    if (firstSignature) {
      pdf.addImage(firstSignature, "jpeg", 145, 157, 10, 5);
      pdf.text(
        `(ลงชื่อ)...............................................`,
        120,
        163
    ...
    pdf.setFont("MyFontBold", "bold");
    pdf.text("คำสั่ง", 140, 210);

    pdf.setFont("MyFont", "normal");
    pdf.addImage(checked, "jpeg", 121, 216, 5.1, 5.1);
    pdf.addImage(blank, "jpeg", 145, 216, 5, 5);
    pdf.text(
      `อนุญาต          ไม่อนุญาต`,
      128,
      220,
    ...
    pdf.addImage(secondSignature, "jpeg", 145,224, 10, 5);
    pdf.text(
      `(ลงชื่อ)...............................................`,
      120,
      230
    ...
    pdf.setFontSize(14);
    pdf.text(
      `               ${leaveData[0].who_second_supeior.split('(')[1].trim().slice(0, -1)}`,
      110,
      244,{ maxWidth: 70 }
    );
    pdf.setFontSize(16);
